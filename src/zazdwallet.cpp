...
CzAZDWallet::CzAZDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zazd seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAZDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAZDSeed_deprecated()) {
                    LogPrintf("%s: Updated zAZD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zazd seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zazd wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAZDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAZDCount(nCountLastUsed);
    else if (!walletdb.ReadZAZDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAZDWallet::Lock()
{
    seedMaster = 0;
}

void CzAZDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAZDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAZD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAZDWallet::LoadMintPoolFromDB()
{
...
void CzAZDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAZDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAZDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zazdTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAZDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAZD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zazdTracker which also adds to database
    pwalletMain->zazdTracker->Add(dMint, true);
...
void CzAZDWallet::SeedToZAZD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAZDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAZDWallet::GenerateDeterministicZAZD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAZDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAZDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
