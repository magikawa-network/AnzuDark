...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZAzdStake(spend));
    } else {
        ...
        CAzdStake* azdInput = new CAzdStake();
        azdInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(azdInput);
    }
