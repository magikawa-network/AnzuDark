...
public:
    ...
    bool WriteZAZDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZAZDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZAZDSeed_deprecated(uint256& seed);
    bool EraseZAZDSeed();
    bool EraseZAZDSeed_deprecated();

    bool WriteZAZDCount(const uint32_t& nCount);
    bool ReadZAZDCount(uint32_t& nCount);
