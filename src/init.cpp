...
#include "config/anzudark-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "anzudark.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "anzudarkd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the AZD and zAZD money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in AZD/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in AZD/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, anzudark, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in AZD/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AnzuDark specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-azdstake=<n>", strprintf(_("Enable or disable staking functionality for AZD inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zazdstake=<n>", strprintf(_("Enable or disable staking functionality for zAZD inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzazd=<n>", strprintf(_("Enable automatic wallet backups triggered after each zAZD minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zazdbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zAZD backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeanzudarkamount=<n>     " + strprintf(_("Keep N AZD anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The AnzuDark Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that AnzuDark is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AnzuDark Core is shutting down."));
...
#endif
    // Make sure only a single AnzuDark process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AnzuDark Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AnzuDark version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //AnzuDark: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //AnzuDark specific: zerocoin and spork DB's
                ...
                // AnzuDark: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZAZDMinted();
                        RecalculateZAZDSpent();
                    }
                    RecalculateAZDSupply(1);
                }
                ...
                // AnzuDark: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AnzuDark Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart AnzuDark Core to complete") << "\n";
        ...
        zwalletMain = new CzAZDWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zAZDWallet
        uiInterface.InitMessage(_("Syncing zAZD wallet..."));

        bool fEnableZAzdBackups = GetBoolArg("-backupzazd", true);
        pwalletMain->setZAzdAutoBackups(fEnableZAzdBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zazdTracker->Init();
...
//    }
//
//    nAnonymizeAnzudarkAmount = GetArg("-anonymizeanzudarkamount", 0);
//    if (nAnonymizeAnzudarkAmount > 999999) nAnonymizeAnzudarkAmount = 999999;
//    if (nAnonymizeAnzudarkAmount < 2) nAnonymizeAnzudarkAmount = 2;
    ...
    LogPrintf("Anonymize AnzuDark Amount %d\n", nAnonymizeAnzudarkAmount);
    ...
    /* Denominations
       ...
       For example:
       1AZD+1000 == (.1AZD+100)*10
       10AZD+10000 == (1AZD+1000)*10
    */
