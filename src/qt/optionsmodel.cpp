...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AZD);
    ...
    if (!settings.contains("nAnonymizeAnzudarkAmount"))
        settings.setValue("nAnonymizeAnzudarkAmount", 1000);

    nAnonymizeAnzudarkAmount = settings.value("nAnonymizeAnzudarkAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAnzudarkAmount"))
        SoftSetArg("-anonymizeanzudarkamount", settings.value("nAnonymizeAnzudarkAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in anzudark.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAnzudarkAmount:
            return QVariant(nAnonymizeAnzudarkAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAnzudarkAmount:
            nAnonymizeAnzudarkAmount = value.toInt();
            settings.setValue("nAnonymizeAnzudarkAmount", nAnonymizeAnzudarkAmount);
            emit anonymizeAnzudarkAmountChanged(nAnonymizeAnzudarkAmount);
            break;
        case CoinControlFeatures:
