...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered anzudark address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 AZD.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal anzudark:URI (anzudark:XyZ...?message=example)
            {
...
static void NotifyzAZDReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzAZDReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzAZDReset.connect(boost::bind(NotifyzAZDReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzAZDReset.disconnect(boost::bind(NotifyzAZDReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zazdTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
