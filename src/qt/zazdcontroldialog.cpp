...
#include "ui_zazdcontroldialog.h"
...
std::set<std::string> ZAzdControlDialog::setSelectedMints;
std::set<CMintMeta> ZAzdControlDialog::setMints;

ZAzdControlDialog::ZAzdControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAzdControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZAzdControlDialog::~ZAzdControlDialog()
{
    delete ui;
}

void ZAzdControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAzdControlDialog::updateList()
{
...
void ZAzdControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAzdControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAzd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAzdControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAzdControlDialog::GetSelectedMints()
{
...
void ZAzdControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
